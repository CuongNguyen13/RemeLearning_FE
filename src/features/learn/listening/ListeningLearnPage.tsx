...
import type { ListeningAttemptResult, ListeningPracticeItem } from "@/types/api"

type PracticeView = "idle" | "runner" | "result"

export function ListeningLearnPage() {
  ...
  function handleGenerate(focusItems?: string[]) {
    return (params: GenerateDialogParams) => {
      generate.mutate(
        { ...params, focusItems, translationLang: i18n.language === "vi" ? "vi" : undefined },
        {
          onSuccess: (item) => {
            setCurrentItem(item)
            setResult(null)
  ...
  // A "Luyện ngay" deep-link from a recommendation card auto-generates a passage targeted at that
  // specific missed keyword, once, without requiring the learner to open the dialog themselves.
  useEffect(() => {
  ...
  return (
    <div className="flex flex-col gap-6">
      <div>
        <h1 className="text-3xl font-semibold tracking-tight">{t("learn.listening.title")}</h1>
      ...
      <Tabs value={tab} onValueChange={(next) => typeof next === "string" && setTab(next)}>
        ...
        <TabsContent value="practice" className="mt-6">
            ...
            <Card>
              <CardContent className="py-10">
                <EmptyState
                  ...
                  title={t("learn.listening.emptyTitle")}
                  description={t("learn.listening.emptyDescription")}
                  action={
                    <GenerateDialog
                      trigger={
                        <Button size="lg">
          ...
          {view === "runner" && currentItem && (
            <div className="flex flex-col gap-4">
              ...
              {currentItem.audioUrl && <ListeningPlayer audioUrl={currentItem.audioUrl} />}
              <ListeningQuestions item={currentItem} onSubmit={handleSubmit} isSubmitting={submit.isPending} />
            </div>
          )}
        ...
        <TabsContent value="history" className="mt-6">
          ...
          {!historyLoading && !historyError && (!history || history.length === 0) && (
            <EmptyState icon={<Ear className="size-6" />} title={t("learn.listening.noHistory")} />
          )}

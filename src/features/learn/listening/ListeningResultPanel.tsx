...
import type { ListeningAttemptQuestionResult } from "@/types/api"

interface ListeningResultPanelProps {
  accuracy: number
...
export function ListeningResultPanel({
  accuracy,
  results,
  transcript,
  translation,
          ...
          >
            ...
            {!result.correct && (
              <p className="pl-6 text-muted-foreground">
                {t("learn.listening.yourAnswerWas", {
                  answer: result.yourAnswer || t("learn.listening.noAnswer"),
                })}
              </p>
            )}
            <p className="pl-6 font-medium text-foreground">
              {t("learn.listening.correctAnswerIs", { answer: result.correctAnswer })}
            </p>
            {result.explanation && <p className="pl-6 text-muted-foreground">{result.explanation}</p>}
          </div>
        ))}
      </div>

      <div className="flex flex-col gap-1.5 rounded-xl bg-muted/60 p-3">
        <p className="text-xs font-medium text-primary">{t("learn.listening.transcript")}</p>
        <p className="text-sm whitespace-pre-line text-foreground/90">{transcript}</p>
        {translation && <p className="text-sm whitespace-pre-line text-muted-foreground">{translation}</p>}
      </div>
